...
`Research Intel` 是一个面向研究者的自托管论文情报系统。它不会把论文跟踪做成“抓一堆、推一堆”的信息流，而是围绕你的研究画像每天挑出一小组高相关论文，并把每篇入选论文沉淀成一个可审阅、可分享、可持续更新的 HTML dossier。
...
- 一条清晰主链：`paper.pdf -> paper-scoped workspace -> tmux-backed Codex -> validated archive`
...
这条主链的目标不是“尽量多产出”，而是把每篇入选论文都做成一个后续还能复查、对照和继续扩写的研究资产。
...
6. 将日报、阅读顺序、方法账本和单篇 HTML 档案落到本地文件系统，供 Web 控制台和 Telegram 继续消费。

## 产物长什么样

Research Intel 的核心产物不是“摘要消息”，而是单篇论文页。每个页面都会把研究动机、问题定义、方法拆解、实验结论、评论与延伸问题组织成一个可读的长期档案。
...
- 运行结果、方法账本和历史记录都保存在本地文件系统里，便于审计和二次开发
...
Research Intel 的默认路径是：让 tmux-backed Codex 围绕 `paper.pdf` 生成并验证单篇论文 HTML。
这个仓库不负责你的 Codex 登录、provider、base URL 或 API key 配置；这些需要先在 Codex CLI 自己的配置层完成。下面这条命令只用于确认你的 CLI/provider 已经就绪。
...
这条路径默认**不会触发 Telegram 推送**，适合第一次试跑主链。
...
第一次建议先用 `daily:no-telegram` 验证主链，再切到真实推送。
